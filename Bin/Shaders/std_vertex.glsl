...
layout (location = 10) in uint inDrawId;
...
void main()
{
	...
	float t = time * 0.1;
	float f = float(inDrawId) / 30.0;

	float st = sin(t*0.5 + f*5.0);
	float ct = cos(t*5.0 + f*5.0);
	...
	gl_Position = matrixViewProj * m * position;
	vsOut.normal = mat3(matrixView * m) * inNormal;
	vsOut.color = mix(color0, color1, fract(j * 313.431));
}