...
layout (binding = 2) uniform sampler2D orientaionTexture;
...
void main()
{
    ...
    float angle = texture(orientaionTexture, texCoord).r * 2.0 * 3.141592;
    mat4 rot = constructRotationMatrix(angle);
    vec4 position = (rot * (vVertex + vec4(0, 0, bendAmount * bendFactor, 0))) + offset;


	position *= vec4(1.0, texture(lengthTexture, texCoord).r * 0.9 + 0.3, 1.0, 1.0);
	gl_Position = mvpMatrix * position;
	color = texture(grasspaletteTexture, texture(grasscolorTexture, texCoord).r) +
